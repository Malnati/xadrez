...
export class RoomsGateway {
  ...
  async create(
    @MessageBody() body: { hostId?: string; clock?: ClockPreset },
  ...
  async join(
    @MessageBody() body: { code: string; guestId?: string },
    @ConnectedSocket() socket: Socket,
  ) {
  ...
  async move(
    @MessageBody() body: { code: string } & Omit<MoveInput, "gameId">,
  ) {
  ...
  resign(@MessageBody() body: { code: string; color: "white" | "black" }) {
    this.server.to(body.code).emit("game:ended", {
      result: body.color === "white" ? "black" : "white",
      reason: "resign",
    });
    return { ok: true };
  }
