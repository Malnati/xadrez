  ...
  async applyMove(input: MoveInput) {
    ...
    try {
      ...
      throw new BadRequestException(
        error instanceof Error ? error.message : "Movimento inválido",
      );
    }
      ...
      data: {
        ...
        clocks: {
          upsert: [
            {
              where: {
                gameId_color: { gameId: parsed.gameId, color: "white" },
              },
            ...
            {
              where: {
                gameId_color: { gameId: parsed.gameId, color: "black" },
              },
  ...
  async resign(gameId: string, color: "white" | "black") {
    ...
    const game = await this.prisma.game.update({
      ...
      data: {
        ...
        result: "resigned",
        pgn: {
          set: `${(await this.getGame(gameId)).pgn}\n${result} venceu por abandono.`,
        },
      },
      include: { moves: true },
    });
